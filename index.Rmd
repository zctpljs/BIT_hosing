...
treat_order <- c("BAU" ,"Good practice", "Keep everything", "Ways to win", "Epic downtime", "No stimulus")
treat_labels <- c("Business as Usual", "Good Practice", "Low Risk to Potential Reward", "Ease of Winning","Fun-framing","No Advert")
...
dt %>% 
    ...
    head() %>% 
    knitr::kable(caption="6 data points suggesting something wrong in the ads trial" %>% stringr::str_to_title())
```
...
## What's the binary % of sessions that win money, excluding free spins?
...
## Sort out the treatment order
treat_order <- c("BAU", "Good practice","Typo - Good Practice",  "No stimulus", "Keep everything", "Ways to win", "Epic downtime")
treat_labels <- c("Business as Usual","Good Practice","Typo - Good Practice" ,"No Advert",  "Low Risk to Potential Reward", "Ease of Winning","Fun-framing")
...
dt %>% 
    ...
    mutate(term=str_remove(term, "treatment")) %>% 
    rename(treatment=term) %>% 
    left_join(y=.,x= dt %>% dplyr::count(treatment), by=join_by(treatment)) %>% 
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption="Play Percentage")
```
...
dt %>% 
    ...
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption = "Unconditional Amount Staked")
```
...
dt %>%
    ...
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption="Robust Poisson Regression for Number of Spins")
```
...
dt[treatment!="No Advert", .(terms_total=mean(termsTotal)), by=treatment] %>% 
    ...
    arrange(treatment) %>% 
    knitr::kable(caption=str_to_title("Average number of T&Cs recalled"))
```
...
dt %>% 
    ...
    knitr::kable(caption=str_to_title("Positive and Negative Sentiment Index"))
```
...
dt %>% 
    ...
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption=str_to_title("OLS Regression of negative sentiment score"))
...
dt %>% 
    ...
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption=str_to_title("OLS Regression of positive sentiment score"))
```
...
dt %>% 
    ...
    arrange(treatment) %>% 
    kable(caption=str_to_title("Sentiment Q on whether advert made them want to play"))
```
...
dt %>% 
    ...
    mutate(term=str_remove(term, "treatment")) %>% 
    rename(treatment=term) %>% 
    left_join(y=.,x= dt %>% dplyr::count(treatment) %>% filter(treatment!="No Advert"), by=join_by(treatment)) %>% 
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption=str_to_title("Logistic Regression of Fruit Rush moderately/very much making them want to play"))
```
...
dt %>% 
    ...
    knitr::kable(caption=str_to_title("Percentage Recognising a gambling advert in their news feed"))
```
...
dt %>% 
   ...
   filter(grepl(term, pattern="treatment")) %>% 
    ...
    select(treatment, n,estimate, p.value, std.error) %>%
    mutate(across(where(is.numeric), function(x){round(x,4)})) %>% 
    knitr::kable(caption=str_to_title("Logistic Regression of recognising gambling advert in socials feed"))
```
...
dt %>%
    ...
    mutate(across(where(is.numeric),function(x){paste0(100*round(x,3),"%")})) %>%
    arrange(treatment) %>% 
    knitr::kable(caption=str_to_title("Percentage regretting playing the game at all"))
```
